import { WorkspaceSchema, McpServerSchema, UpdateWorkspaceSchema } from '@waylaidwanderer/sumika-types';
import { z } from 'zod';
...
workspaceRoutes.openapi(createRoute({
  ...
  try {
    const newWorkspace = await workspaceManager.createWorkspace(name, description, path);
    ...
    logger.error({ err: error, name, description, customPath: path }, 'Failed to create workspace');
...
workspaceRoutes.openapi(createRoute({
  ...
  const { workspaceId } = c.req.valid('param');
  try {
    const isEmpty = await workspaceManager.isWorkspaceEmpty(workspaceId);
    return c.json({ isEmpty });
  } catch (error) {
...
workspaceRoutes.openapi(createRoute({
  ...
  const updatedWorkspace = await workspaceManager.updateWorkspace(workspaceId, updates);
  if (!updatedWorkspace) {
    return c.json({ error: 'Workspace not found' }, 404);
  }
...
workspaceRoutes.openapi(createRoute({
  ...
  try {
    const success = await workspaceManager.deleteWorkspace(workspaceId, deleteFiles || false);
    if (!success) {
      return c.json({ error: 'Workspace not found' }, 404);
    }
...
workspaceRoutes.openapi(FileExistsRoute, async (c) => {
    ...
    const filePath = path.join(workspace.path, sanitizedFilename);
...
workspaceRoutes.openapi(UploadFileRoute, async (c) => {
    ...
    const destinationPath = path.join(workspace.path, sanitizedFilename);
    ...
    try {
        ...
        console.error(`Error saving uploaded file for workspace ${workspaceId}:`, error);
