...
import { sessionManager, workspaceManager } from '../managers';
import { SessionDataSchema, PromptContentSchema, TextContent, ResourceContent } from '@waylaidwanderer/sumika-types';
import { logger } from '../logger';
import { SessionIdSchema, ErrorSchema } from '../api-types';
...
sessionRoutes.openapi(GetSessionsRoute, (c) => {
  const { workspaceId, limit, offset, view } = c.req.valid('query');
  const sessions = sessionManager.getAllSessions({ workspaceId, limit, offset, view });
...
sessionRoutes.openapi(CreateSessionRoute, async (c) => {
  try {
    ...
    const sessionId = await sessionManager.createSession(workspaceId);
...
sessionRoutes.openapi(UpdateSessionRoute, async (c) => {
  ...
  const updatedSession = sessionManager.updateSession(sessionId, updates);
  if (!updatedSession) {
    return c.json({ error: 'Session not found' }, 404);
  }
...
sessionRoutes.openapi(ExportSessionRoute, (c) => {
  const { sessionId } = c.req.valid('param');
  const markdown = sessionManager.exportSessionToMarkdown(sessionId);
  if (markdown === '') {
    return c.json({ error: 'Session not found' }, 404);
  }
  
  const sessionName = sessionManager.getSession(sessionId)?.name || 'session';
...
sessionRoutes.openapi(BranchSessionRoute, async (c) => {
    ...
    try {
        const newSession = await sessionManager.branchSession(sessionId, branchFromMessageId);
...
sessionRoutes.openapi(ListenRoute, async (c) => {
  ...
  return streamSSE(c, async (stream) => {
    sessionManager.registerStream(sessionId, stream);
...
const PromptRoute = createRoute({
    ...
    responses: {
        201: { 
            ...
            content: { 'application/json': { schema: z.any().openapi({ type: 'object', description: 'The created UserMessage object' }) } } 
        },
...
sessionRoutes.openapi(PromptRoute, async (c) => {
    ...
    const finalContent = await sessionManager.processFileMentions(session.workspaceId, content);

    const newUserMessage = sessionManager.addUserMessage(sessionId, finalContent);
    sessionManager.prompt(sessionId, finalContent);
...
sessionRoutes.openapi(PermissionRoute, async (c) => {
    ...
    sessionManager.sendPermissionResponse(requestId, outcome);
    sessionManager.updatePermissionRequestMessage(sessionId, requestId, outcome.optionId);
...
sessionRoutes.openapi(ReloadSessionRoute, async (c) => {
    ...
    try {
        const newId = await sessionManager.reloadSession(sessionId);
        if (!newId) {
            return c.json({ error: 'Session not found' }, 404);
        }
...
sessionRoutes.openapi(CompressSessionRoute, async (c) => {
    ...
    try {
        const success = await sessionManager.compressHistory(sessionId);
        if (success) {
